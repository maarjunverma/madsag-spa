...
const ServiceDetailView: React.FC<ServiceDetailViewProps> = ({ service, onClose, onEnquire }) => {
  return (
    <div className="fixed inset-0 z-[150] bg-[#020617] overflow-y-auto animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-[1200px] h-[600px] bg-gradient-to-r ${service.gradient} opacity-10 blur-[180px] rounded-full -z-20`}></div>
      ...
      <div className="sticky top-0 z-50 glass px-6 py-4 flex justify-between items-center border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span className="font-black text-xl tracking-tighter uppercase">{service.name}</span>
        </div>
        <button 
          onClick={onClose}
        ...
        <div className="grid lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-8 space-y-24">
            ...
            {service.packages && (
              <section>
                <div className="mb-12">
                  <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-amber-500 mb-4">Investment Matrix</h2>
                  <h3 className="text-4xl font-black tracking-tighter uppercase leading-none">STRATEGIC <span className="text-gold">TIERS</span></h3>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {service.packages.map((pkg, idx) => (
                    <div 
                      key={idx} 
                      className={`relative glass rounded-[2.5rem] p-8 flex flex-col transition-all duration-500 border-white/5 group hover:border-amber-500/30 ${
                        pkg.recommended ? 'ring-2 ring-amber-500/20 md:scale-105 bg-white/[0.03]' : ''
                      }`}
                    >
                      {pkg.recommended && (
            ...
            {/* Strategic Roadmap */}
            <section className="bg-white/5 border border-white/10 rounded-[3rem] p-10 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 blur-3xl"></div>
              <h2 className="text-2xl font-black mb-10 flex items-center gap-3 uppercase">
                <i className="fa-solid fa-map text-amber-500"></i>
                STRATEGIC ROADMAP: <span className="text-amber-500">PLANNING TO LAUNCH</span>
              </h2>
                ...
                {service.roadmap.map((item, idx) => (
                  <div key={idx} className="flex gap-6 items-start group">
                    <div className="flex flex-col items-center">
                      ...
                      {idx < service.roadmap.length - 1 && <div className="w-px h-16 bg-white/10 my-2"></div>}
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-xs font-black uppercase text-amber-500 tracking-widest">{item.phase}</span>
                      ...
                      <p className="text-gray-300 font-bold group-hover:text-white transition-colors">{item.objective}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-32 space-y-8">
              <div className="glass p-8 rounded-[2rem] border-white/5">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-amber-500 mb-8">Performance Metrics</h3>
                <div className="space-y-8">
                ...
                <div className="mt-12 pt-8 border-t border-white/10">
                  <button 
                    onClick={() => onEnquire()}
                    className={`w-full py-5 bg-gradient-to-r ${service.gradient} text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-2xl transition-all hover:scale-105`}
                  >
                    Custom Briefing
                  </button>
                </div>
