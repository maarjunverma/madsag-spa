...
const ServiceSection: React.FC<ServiceSectionProps> = ({ service, isActive, onEnquire, onViewPortfolio, onViewDetails }) => {
  return (
    <section 
      id={service.id} 
      className={`min-h-screen flex flex-col justify-center py-12 md:py-24 px-6 transition-all duration-700 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-40 translate-y-10'}`}
    >
      <div className="max-w-6xl mx-auto w-full grid lg:grid-cols-2 gap-10 md:gap-16 items-center">
        <div className="order-2 lg:order-1">
          <div className="flex items-center gap-4 mb-6 md:mb-8">
            <div className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-[#0a0a0a] border border-white/10 flex items-center justify-center shadow-2xl transition-transform duration-500 ${isActive ? 'scale-110 border-amber-500/50' : 'scale-100'}`}>
              <i className={`${service.icon} text-2xl md:text-3xl text-white`}></i>
            </div>
            <span className="text-[8px] md:text-[10px] px-3 py-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-full font-black uppercase tracking-widest">Active Engineering</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-4">
            <button 
              ...
              className="w-full sm:w-auto px-8 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest text-white transition-all bg-[#0a0a0a] border border-white/10 hover:border-amber-500/50 hover:bg-black shadow-xl shadow-black/50"
            >
              Explore Architecture
            </button>
            <button 
              onClick={onEnquire}
              className="w-full sm:w-auto px-8 py-4 bg-transparent glass border-white/5 rounded-xl font-black uppercase text-[10px] tracking-widest text-white hover:bg-white/5 transition-all"
            >
              Start Conversation
            </button>
          </div>
